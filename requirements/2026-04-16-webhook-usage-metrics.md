...
2. `udp_flow_end` webhook payload MUST include the same five fields, with the
   same semantics (bytes_sent = client→upstream, bytes_received = upstream→client).

3. The five new fields MUST be present on the `*_end` events only (`tcp_conn_end`,
   `udp_flow_end`). Start events (`tcp_conn_start`, `udp_flow_start`) MUST NOT include
   them (values are not yet meaningful at start time).
...
- [ ] `tcp_conn_end` payload contains `bytes_sent`, `bytes_received`, `duration_ms`,
      `listen_port`, and `upstream_addr` with correct values.
- [ ] `udp_flow_end` payload contains the same five fields with correct values.
- [ ] `tcp_conn_start` and `udp_flow_start` payloads do NOT contain the new fields.
